...
'''from transformer import MLPTransformer
class MLP(nn.Module):
    def __init__(self, layer_vec, layer_norm=True):
        super().__init__()
        self.layers = nn.ModuleList()

        problem_dim = layer_vec[0]
        output_dim = layer_vec[-1]
        
        self.transformer = MLPTransformer(
...
class CG_model(torch.nn.Module):
    ...
    def forward(self, x, edge_index, dataset, train = False):
        ...
        grad_W_ij = grad(W_ij, r_ij)
        
        r_ji = -r_ij

        W_ji = self.model_W(r_ji)
        grad_W_ji = grad(W_ji, r_ji)
        ...
        term = P[j] / d[j]**2 * grad_W_ji - P[i] / d[i]**2 * grad_W_ij
        gradP_d = scatter_add(term, i, dim=0, dim_size=N) - scatter_add(term, j, dim=0, dim_size=N)

        # Dissipative dynamics (induced by dx_tilde)
        T_i = T[i]
        T_j = T[j]

        A_ij = self.model_A(r_/self.h, T_i) * self.model_A(r_/self.h, T_j)*1.5
        B_ij = self.model_B(r_/self.h, T_i) * self.model_B(r_/self.h, T_j)*1.5
        C_ij = self.model_C(r_/self.h, T_i) * self.model_C(r_/self.h, T_j)*1.5
